...
object DetectExpandRemain {
  def detectExpandRemain (target25: String, refCloud : String, refWater: String, targetCloud: String, targetWater: String, rb0Path: String, rb1Path: String, rb2Path: String, rb3Path: String, eb0Path: String, eb1Path: String, eb2Path: String, eb3Path: String): Array[Array[Array[Double]]] = {
    ...
    val tiffRemainB0 = DoubleArrayTile(remainDataBand(0), Xsize, Ysize).convert(FloatCellType)
    SinglebandGeoTiff(tiffRemainB0, geoTiffMul.extent, geoTiffMul.crs).write(rb0Path)
    val tiffRemainB1 = DoubleArrayTile(remainDataBand(1), Xsize, Ysize).convert(FloatCellType)
    SinglebandGeoTiff(tiffRemainB1, geoTiffMul.extent, geoTiffMul.crs).write(rb1Path)
    val tiffRemainB2 = DoubleArrayTile(remainDataBand(2), Xsize, Ysize).convert(FloatCellType)
    SinglebandGeoTiff(tiffRemainB2, geoTiffMul.extent, geoTiffMul.crs).write(rb2Path)
    val tiffRemainB3 = DoubleArrayTile(remainDataBand(3), Xsize, Ysize).convert(FloatCellType)
    SinglebandGeoTiff(tiffRemainB3, geoTiffMul.extent, geoTiffMul.crs).write(rb3Path)
